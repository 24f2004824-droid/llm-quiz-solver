...
export default async function handler(req, res) {
  ...
  res.status(200).json({ status: 'accepted', started: new Date().toISOString() });

  // Fire and forget — solve the whole chain
  solveChain({ email, secret, url }).catch(err => console.error('Solver error:', err));
}
