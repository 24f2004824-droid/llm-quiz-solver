...
async function getBrowser() {
  ...
  // Official tar URL for version 130.0.0 (forces download/extract to /tmp/chromium, fixes brotli error)
  const tarUrl = 'https://github.com/Sparticuz/chromium-min/releases/download/v130.0.0/chromium-130.0.0.tar.bz2';

  browser = await puppeteer.launch({
    headless: 'new',
    executablePath: await chromium.executablePath(tarUrl),  // Pass tar URL here — extracts brotli files automatically
    args: [
      ...chromium.args,  // Auto-includes --no-sandbox, --disable-setuid-sandbox, etc.
      '--disable-gpu',
